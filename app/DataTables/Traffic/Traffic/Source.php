...
class Source extends DataTable
{
    ...
    public function ajax($export = false, $take = 1000, $time = false)
    {
        ...
        if (isset($action) && in_array($action, ['print', 'excel'])) {
            $obj->setTransformer('App\Helper\Export\Traffic\Traffic\Source');
        } else {
            $obj->setTransformer('App\Helper\Table\Traffic\Traffic\Source');
        }
        //构造查询条件
        ...
        $obj = $obj->groupBy('Source_Id')->groupBy('Insert_Date');
